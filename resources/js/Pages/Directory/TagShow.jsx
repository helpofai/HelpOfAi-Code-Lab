...
import { Tag, ChevronRight, Hash, Eye, ShoppingBag, ArrowRight } from 'lucide-react';
import ProjectPreviewContent from '@/Components/ProjectPreviewContent';

export default function TagShow({ tag, projects }) {
    return (
        <PublicLayout>
            <Head title={`#${tag} Projects`} />
            ...
            <div className="relative pt-32 pb-20 overflow-hidden bg-[var(--bg-main)] border-b border-[var(--border)]">
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10">
                    <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] mb-6">
                        <Link href={route('public.tags.index')} className="hover:text-purple-500 transition-colors">Tags</Link>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-[var(--text-muted)] text-sm md:text-base font-medium">
                                Browse {projects.total} projects tagged with #{tag}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-20 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {projects.data.map((project, idx) => (
                        <motion.div
                            ...
                            className="group bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl overflow-hidden hover:border-purple-500/50 hover:shadow-2xl transition-all duration-300 flex flex-col"
                        >
                            ...
                            <div className="p-5 flex-1 flex flex-col">
                                <h3 className="font-bold text-[var(--text-main)] leading-tight mb-2 line-clamp-2 group-hover:text-purple-500 transition-colors">
                                    {project.title}
                                </h3>
                ...
                {projects.data.length === 0 && (
                    <div className="text-center py-32 bg-[var(--bg-surface)] rounded-2xl border border-[var(--border)]">
                        ...
                        <p className="text-[var(--text-muted)] mt-2 font-medium">There are no projects with this tag yet.</p>
                        <Link href={route('public.tags.index')} className="mt-6 btn-secondary inline-flex items-center gap-2">
                            <ArrowRight size={16} /> Browse Other Tags
                        </Link>
                    </div>
                )}
                        ...
                        {projects.links.map((link, k) => (
                            <Link
                                ...
                                className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest rounded transition-all ${
                                    link.active 
                                    ? 'bg-purple-500 text-black dark:text-white border-transparent' 
                                    : 'bg-[var(--bg-surface)] border border-[var(--border)] text-[var(--text-muted)] hover:text-purple-500'
