                ...
                header={
                    <div className="flex justify-between items-center w-full">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h2 className="text-xl font-black text-white tracking-tighter uppercase leading-tight italic">My_Neural_Cores</h2>
                        ...
                        <div className="flex items-center space-x-6">
                            <div className="relative hidden md:block">
                                ...
                                <input 
                                    ...
                                    className="bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2 text-[10px] font-black uppercase tracking-widest focus:border-cyan-500/50 focus:ring-0 w-72 transition-all"
                                />
                            </div>
                            <Link
                                href={route('editor')}
                                className="flex items-center px-8 py-3 bg-cyan-500 text-black rounded-lg font-black text-[10px] uppercase tracking-[0.3em] hover:bg-white transition-all shadow-[0_0_20px_rgba(34,211,238,0.3)]"
                            >
                        ...
                        <AnimatePresence mode="wait">
                            {isLoading ? (
                                ...
                                <motion.div 
                                    ...
                                    className="bg-white/[0.02] border border-white/10 rounded-[4rem] p-32 text-center backdrop-blur-3xl"
                                >
                                    <Terminal className="w-24 h-24 text-cyan-500/10 mx-auto mb-10" />
                                    <h3 className="text-3xl font-black text-white mb-4 uppercase tracking-tighter italic">Archives_Empty</h3>
                                    <p className="text-white/30 font-bold uppercase tracking-widest text-xs mb-12">No active neural modules found in current sector.</p>
                                    <Link href={route('editor')} className="px-12 py-5 bg-white text-black font-black rounded-xl hover:bg-cyan-400 transition-all uppercase tracking-widest text-[11px]">Deploy_Module_01</Link>
                                </motion.div>
                            ) : (
                                <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-12">
                                    {filteredProjects.map((project, idx) => (
                                        <motion.div
                                            ...
                                            className="group relative bg-[#0f172a]/40 border border-white/10 rounded-[2.5rem] overflow-hidden hover:border-cyan-500/50 hover:shadow-[0_0_80px_rgba(6,182,212,0.15)] transition-all duration-700"
                                        >
                                            <div className="aspect-[16/10] w-full bg-black relative overflow-hidden border-b border-white/5">
                                                <ProjectThumbnail project={project} />
                                                <div className={`absolute top-6 right-6 px-4 py-1.5 rounded-full backdrop-blur-xl border font-black text-[9px] uppercase tracking-widest flex items-center space-x-2 z-20 ${project.is_public ? 'bg-cyan-500/10 border-cyan-500/30 text-cyan-400' : 'bg-rose-500/10 border-rose-500/30 text-rose-400'}`}>
                                            ...
                                            <div className="p-10 text-left">
                                                <div className="flex justify-between items-start mb-8">
                                                    <div className="flex-1 min-w-0">
                                                        <h3 className="text-2xl font-black text-white uppercase tracking-tighter truncate group-hover:text-cyan-400 transition-colors italic">{project.title}</h3>
                                                        <div className="flex items-center mt-3 space-x-6 opacity-40">
                                                            <div className="flex items-center text-[10px] font-black text-white uppercase tracking-[0.2em]"><Clock size={14} className="mr-2 text-cyan-500" /> {new Date(project.updated_at).toLocaleDateString()}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                <div className="flex items-center space-x-4 mb-8">
                                                    <Link href={route('editor', { slug: project.slug })} className="flex-1 flex items-center justify-center space-x-3 py-4 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-cyan-400 transition-all shadow-xl">
                                                        <ExternalLink size={18} />
                                                        <span>Open_Editor</span>
                                                    </Link>
                                                    <button onClick={() => setEditingProject(project)} className="p-4 bg-white/5 text-white border border-white/10 rounded-2xl hover:bg-white/10 transition-all" title="Configure Module">
                                                        <Settings size={20} />
                                                    </button>
                                                    <button onClick={() => handleShare(project.slug)} className="p-4 bg-white/5 text-white border border-white/10 rounded-2xl hover:bg-white/10 transition-all" title="Copy Link">
                                                        {copySlug === project.slug ? <Check size={20} className="text-green-400" /> : <Share2 size={20} />}
                                                    </button>
                                                    <button onClick={() => handleDelete(project.id)} className="p-4 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-2xl hover:bg-rose-500 hover:text-white transition-all" title="Delete">
                                                        <Trash2 size={20} />
                                                    </button>
                                                </div>
                                                
                                                <div className="flex bg-white/[0.02] border border-white/5 rounded-2xl p-4 items-center justify-between">
                                                    <span className="text-[9px] font-black text-white/30 uppercase tracking-[0.3em]">Access_Path</span>
                                                    <code className="text-[10px] text-cyan-500/60 font-mono">/archives/{project.slug.slice(0, 12)}...</code>
                                                </div>
                                            </div>                                        
                                        </motion.div>
                                    ))}
